...
- **In flight:** Phase 2 (`niles-mqtt` is read-only — command publishing next), Phase 6 (scenes, morning routine still pending)
...
- **Last PR merged:** [#11 — niles-mqtt auto-reconnect](https://github.com/MarkNygaard/niles/pull/11)
...
- 🚧 `niles-mqtt` — connection + Z2M parser ([#9](https://github.com/MarkNygaard/niles/pull/9)) + registry wiring via `Z2mSource` ([#10](https://github.com/MarkNygaard/niles/pull/10)) + auto-reconnect with subscription replay ([#11](https://github.com/MarkNygaard/niles/pull/11)). Command publishing still pending.
