...
TfLiteStatus InitializeMicroFeatures() {
  ...
  TfLiteTensor* pin = interpreter->input(0);
  TfLiteTensor* pout = interpreter->output(0);
  MicroPrintf("preproc: in_type=%d in_elems=%d | out_type=%d out_elems=%d out_scale=%f out_zp=%d",
              (int)pin->type, (int)(pin->bytes / sizeof(int16_t)), (int)pout->type,
              (int)pout->bytes, (double)pout->params.scale, (int)pout->params.zero_point);
