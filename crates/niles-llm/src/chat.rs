//! Shared OpenAI-shaped chat-completions wire types.
//!
//! Used by both Groq and OpenAI backends. Request and response types
//! are the stable public surface; private wire types may evolve as
//! providers diverge.
...
pub(crate) struct RawToolCall {
    pub(crate) id: String,
    pub(crate) function: RawFunctionCall,
}
...
/// POST a [`ChatRequest`] to `{base_url}/chat/completions` and parse
/// the response. This is the shared core used by both Groq and OpenAI
/// clients — the only provider-specific differences are the URL,
...
pub(crate) async fn post_chat_completions(
    ...
    if !status.is_success() {
        ...
        let preview = &body_bytes[..body_bytes.len().min(MAX_ERR_BODY)];
...
mod tests {
    ...
    fn chat_response_empty_choices_is_invalid_response() {
        let body = br#"{"choices":[]}"#;
        let raw: RawChatResponse = serde_json::from_slice(body).unwrap();
        // post_chat_completions turns this into Error::InvalidResponse — the parse
    ...
    fn provider_error_truncates_long_body() {
        ...
        const MAX_ERR_BODY: usize = 2048;
        let preview = &long_body[..long_body.len().min(MAX_ERR_BODY)];
